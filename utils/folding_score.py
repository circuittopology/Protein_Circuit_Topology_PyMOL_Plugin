...
def get_folding_score(
    ...
    # Calculate averages for P, S, and X across the trajectory
    ...
    normalized_parallel = sum(parallel_relations) / avg_parallel if avg_parallel != 0 and len(parallel_relations) > 0 else 0
    normalized_series = sum(series_relations) / avg_series if avg_series != 0 and len(series_relations) > 0 else 0
    normalized_crossover = sum(crossover_relations) / avg_crossover if avg_crossover != 0 and len(crossover_relations) > 0 else 0
