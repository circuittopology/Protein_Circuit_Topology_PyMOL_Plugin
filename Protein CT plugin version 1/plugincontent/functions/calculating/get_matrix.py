...
def get_matrix(index: np.ndarray, protid: str) -> Union[Tuple[np.ndarray, List[Union[str, int, float]]], Tuple[np.ndarray, List[Union[str, int]], Dict[str, Dict[str, int]]]]:
    """
    ...
    Returns:
        A tuple depending on the input shape. 
        For shape (N, 2), returns a matrix and a list of stats [protid, P, S, X, P_ratio, S_ratio, X_ratio].
        For shape (N, 4), returns a matrix, a list of stats [protid, P, S, X, I2, I3, I4, T2, T3, L], 
        and a dictionary of chain stats.
    """
    if index.shape == (0,):
    ...
    if np.shape(index)[1] == 2:
        ...
        chainstats = {}
        for i in chainids:
            chainstats[str(i)] = {'p':0,'s':0,'x':0,'i2':0,'i3':0,'i4':0,'t2':0,'t3':0,'l':0}
        ...
        for x in range(len(index)):
            ...
            i = index[x][0]
            j = index[x][1]
            chaini = str(index[x][2])
            chainj = str(index[x][3])
            ...
            for y in range(x+1,len(index)):
                ...
                k = index[y][0]
                l = index[y][1]
                chaink = str(index[y][2])
                chainl = str(index[y][3])
    ...
    return np.zeros((len(index), len(index)), dtype='int'), [protid, 0, 0, 0]